...
int main(void) {
	...
	while(fgets(buffer1, sizeof(buffer1), file)) {
		for (int i = 0; buffer1[i] != '\0'; i++) {
			if (buffer1[i] == 'm' && buffer1[i+1] == 'u' && buffer1[i+2] == 'l' && buffer1[i+3] == '(') {
				int j = 0;
	...
	while(fgets(buffer2, sizeof(buffer2), file)) {
		for (int i = 0; buffer2[i] != '\0'; i++) {
			if (buffer2[i] == 'm' && buffer2[i+1] == 'u' && buffer2[i+2] == 'l' && buffer2[i+3] == '(') {
				int j = 0;
				bool isDigitsVal1 = true;
			...
			if (buffer2[i] == 'd' && buffer2[i+1] == 'o' && buffer2[i+2] == 'n' && buffer2[i+3] == '\'' && buffer2[i+4] == 't' && buffer2[i+5] == '(' && buffer2[i+6] == ')') {
				enabled = false;
			}
			if (buffer2[i] == 'd' && buffer2[i+1] == 'o' && buffer2[i+2] == '(' && buffer2[i+3] == ')') {
				enabled = true;
			}
		}
	}
